...
class Facility(models.Model):
    estate = models.ForeignKey('Residents.Estate', on_delete=models.CASCADE)
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, null=True)
    booking_slot_configuration = models.JSONField()  # Booking configuration details
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    billing = models.OneToOneField('Facilities.Billing', on_delete=models.SET_NULL, null=True, blank=True, related_name='facility_detail')
